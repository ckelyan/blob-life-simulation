...
class Blob:
    def __init__(self, x, y, grid_size):
        ...
        self.age = 0
        self.death_age = int(np.random.normal(params["expected_death"][self.sex],params["death_variance"]))
    ...
    def move(self, changex, changey):
        ...
        if moved:
            self.energy += int(np.sqrt(changex**2 + changey**2)*params["travel_energy"])
...
class Sim:
    ...
    def initgrid(self):
        self.blobpos = list(map(lambda x: [x // self.size, x % self.size], np.random.randint(0, self.size**2, self.nblobs)))
        self.blobs = [Blob(*self.blobpos[i], self.size) for i in range(self.nblobs)]
        self.foodpos = list(map(lambda x: [x // self.size, x % self.size], np.random.randint(0, self.size**2, self.nfood)))
    ...
    def updatevalues(self):
        ...
        for blob in self.blobs:
            ...
            blob.move(np.random.randint(*blob.available_moves(blob.x)) - params["max_travel"], np.random.randint(*blob.available_moves(blob.y)) - params["max_travel"])
        ...
        for y in range(self.size):
            for x in range(self.size):
                if [x,y] in self.blobpos:
                    males = [blob for blob in self.blobs if ((blob.x == x) and (blob.y == y) and (blob.sex == "m"))]
                    females = [blob for blob in self.blobs if ((blob.x == x) and (blob.y == y) and (blob.sex == "f"))]
                    while len(males)>1:
                        sorted_males = sorted(males, key=lambda x: x.energy)
                        weak_male = sorted_males[0]
                        winning_males = sorted_males[1:]
                        winning_males_energy = sum(blob.energy for blob in winning_males)
                        weak_male.dead = True
                        added_energy = [weak_male.energy*params["death_energy_factor"] * blob.energy/winning_males_energy for blob in winning_males]
                    ...
                    if males:
                        ...
                        for female in females:
                            female.dead = np.random.binomial(1,params["pregnancy_death_chance"])
                            num_children += np.random.geometric(1-params["birth_chance"])
                            self.births += num_children
                        for k in range(num_children):
                            child = Blob(x+np.random.randint(-1,1),y+np.random.randint(-1,1), self.size)
        ...
        self.blobpos = list(map(lambda b: [b.x, b.y], self.blobs))
        self.mpos = list(map(lambda b: [b.x, b.y] if (b.sex=="m") else None, self.blobs))
        self.mpos = list(filter(None, self.mpos))
        self.fpos = list(map(lambda b: [b.x, b.y] if (b.sex=="f") else None, self.blobs))
        self.fpos = list(filter(None, self.fpos))
        
        self.update_food()
    
    def update_food(self):
        self.foodpos = [food for food in list(self.foodpos) if food not in list(self.blobpos)]
        self.foodpos.extend(list(map(lambda x: [x // self.size, x % self.size], np.random.randint(0, self.size**2, int(len(self.foodpos)*params["food_regeneration"])))))
    ...
    def plotgrid(self):
        ...
        plt.gcf().text(0.02,0.5,f"Statistics\n\nBlobs: {len(self.blobs)}\n\nMales: {len(self.mpos)}\n\nFemales: {len(self.fpos)}\n\nBirths: {self.births}\n\nDeaths: {self.deaths}\n\nFood: {len(self.foodpos)}\n\nAverage energy: ~{int(total_energy)}",fontsize=14)
        #for i, blob in enumerate(self.blobs):
